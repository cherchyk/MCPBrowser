...
async function main() {
  const server = new Server({ name: "MCP Browser", version: "0.1.0" }, { capabilities: { tools: {} } });

  const tools = [
    {
      name: "load_and_extract",
      description: "Load an authenticated page via local Chrome (DevTools) and return text+html. Default settings wait for network idle and handle auth redirects automatically. If page is loading, the tool waits—no need to retry manually.",
      inputSchema: {
        type: "object",
        properties: {
          url: { type: "string" },
