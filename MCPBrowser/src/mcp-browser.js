...
import { clickElement, typeText, getInteractiveElements, waitForElement } from './actions.js';
...
async function main() {
  ...
  const tools = [
    ...
    {
      name: "click_element",
      description: "Clicks on an element on the page. Works with any clickable element including buttons, links, or elements with onclick handlers. Can target by CSS selector or text content. The page must be already loaded via fetch_webpage_protected first.",
      inputSchema: {
        type: "object",
        properties: {
          ...
          timeout: { type: "number", description: "Maximum time to wait for element in milliseconds", default: 30000 }
        },
        required: ["url"],
        additionalProperties: false,
      },
    ...
    {
      name: "type_text",
      description: "Types text into an input field, textarea, or other editable element. The page must be already loaded via fetch_webpage_protected first.",
      inputSchema: {
        type: "object",
        properties: {
          url: { type: "string", description: "The URL of the page (must match a previously fetched page)" },
    ...
    },
    {
      ...
      description: "Lists all interactive elements on the page including links, buttons, inputs, and elements with onclick handlers. Useful for discovering what can be clicked or interacted with. The page must be already loaded via fetch_webpage_protected first.",
      inputSchema: {
        type: "object",
        properties: {
          url: { type: "string", description: "The URL of the page (must match a previously fetched page)" },
    ...
    },
    {
      ...
      description: "Waits for an element to appear on the page. Useful after clicking something that triggers dynamic content loading. Can wait for element by CSS selector or text content. The page must be already loaded via fetch_webpage_protected first.",
      inputSchema: {
        type: "object",
        properties: {
          url: { type: "string", description: "The URL of the page (must match a previously fetched page)" },
